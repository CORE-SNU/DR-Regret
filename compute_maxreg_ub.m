...
while UB - LB > err_tol 
    ...
    if L == 1 
        ...
        Ain_master = [ 
            -(1/N)*sum(q_plus + q_minus) -1; 
            ];
        bin_master = [
            -(1/N)*(kron(ones(1,N), eye(r))*nu)'*C*x + (1/N)*a'*kron(ones(1,N), eye(n_2))*z - (1/N)*(xi_hat_vec+q_plus-q_minus)'*kron(I_N, E')*nu - (1/N)*b'*kron(ones(1,N), eye(r))*nu
            ];
        lb_master = [
            0;
            -inf;
    ...
    f_sub = [
        zeros(r*N + N*n_2 + n, 1); 
        (1/N) * kron(ones(N,1),a); % z^{y}
        (1/N) * lambda * ones(N*m,1);
        (1/N) * lambda * ones(N*m,1);
        -(1/N) * kron(ones(N,1),a); % z^{x}
        zeros(r*N,1)
        ]; 
    lb_sub = [ 
    ...
    z = sol_sub(r*N + N*n_2 + n + 1 : r*N + N*n_2 + n + N*n_2);
    q_plus = sol_sub(r*N + N*n_2 + n + N*n_2 + 1 : r*N + N*n_2 + n + N*n_2 + N*m);
    q_minus = sol_sub(r*N + N*n_2 + n + N*n_2 + N*m + 1 : r*N + N*n_2 + n + N*n_2 + N*m + N*m);
    nu = sol_sub( r*N + N*n_2 + n + N*n_2 + 2*N*m + N*n_2 + 1 : r*N + N*n_2 + n + N*n_2 + 2*N*m + N*n_2 + r*N);
