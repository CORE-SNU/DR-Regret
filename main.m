...
% dro_exantereg_CCG 
sol_dro_exantereg_CCG = cell(n_simu,n_eps); 
maxreg_ub_dro_exantereg_CCG = zeros(n_simu,n_eps); 
for i_simu = 1 : n_simu 
    xi_hat_matrix = xi_matrix_true(sample_index{i_simu},:); 
    xi_hat_vec = reshape(xi_hat_matrix',[],1);    
    for i_eps = 1 : n_eps
        Was_dist = eps_set(i_eps);
        solution_dro_exantereg_CCG
        sol_dro_exantereg_CCG{i_simu,i_eps} = x;
        maxreg_ub_dro_exantereg_CCG(i_simu,i_eps) = UB;
    end
end
maxreg_ub_dro_exantereg_CCG = mean(maxreg_ub_dro_exantereg_CCG);

% dro_expostreg
sol_dro_expostreg = cell(n_simu,n_eps);
obj_dro_expostreg = zeros(n_simu,n_eps);
for i_simu = 1 : n_simu 
    xi_hat_matrix = xi_matrix_true(sample_index{i_simu},:); 
    xi_hat_vec = reshape(xi_hat_matrix',[],1);    
    for i_eps = 1 : n_eps
        ...
        obj_dro_expostreg(i_simu,i_eps) = UB;
    end
end
% dro_expostreg_CCG
sol_dro_expostreg_CCG = cell(n_simu,n_eps);
obj_dro_expostreg_CCG = zeros(n_simu,n_eps);
for i_simu = 1 : n_simu 
    xi_hat_matrix = xi_matrix_true(sample_index{i_simu},:); 
    xi_hat_vec = reshape(xi_hat_matrix',[],1);    
    for i_eps = 1 : n_eps
        Was_dist = eps_set(i_eps);
        solution_dro_expostreg_CCG
        sol_dro_expostreg_CCG{i_simu,i_eps} = x;
        obj_dro_expostreg_CCG(i_simu,i_eps) = UB;
    end
end
...
% dro_expostreg_CCG
maxreg_ub_dro_expostreg_CCG = zeros(n_simu,n_eps);
for i_simu = 1 : n_simu
    xi_hat_matrix = xi_matrix_true(sample_index{i_simu},:); 
    xi_hat_vec = reshape(xi_hat_matrix',[],1);            
    for i_eps = 1 : n_eps
        x = sol_dro_expostreg{i_simu,i_eps}; 
        Was_dist = eps_set(i_eps);
        compute_maxreg_ub_CCG
        maxreg_ub_dro_expostreg_CCG(i_simu,i_eps) = ub_maxreg;       
    end
end
maxreg_ub_dro_expostreg_CCG = mean(maxreg_ub_dro_expostreg_CCG);        
...
% dro_cost_CCG
maxreg_ub_dro_cost_CCG = zeros(n_simu,n_eps);
for i_simu = 1 : n_simu
    xi_hat_matrix = xi_matrix_true(sample_index{i_simu},:); 
    xi_hat_vec = reshape(xi_hat_matrix',[],1);                
    for i_eps = 1 : n_eps
        ...
        Was_dist = eps_set(i_eps); 
        compute_maxreg_ub
        maxreg_ub_dro_cost_CCG(i_simu,i_eps) = ub_maxreg;       
    end
end
maxreg_ub_dro_cost_CCG = mean(maxreg_ub_dro_cost_CCG); 
...
% dro_exantereg_CCG
maxreg_lb_dro_exantereg_CCG = zeros(n_simu,n_eps);        
for i_simu = 1 : n_simu    
    xi_hat_matrix = xi_matrix_true(sample_index{i_simu},:); 
    xi_hat_vec = reshape(xi_hat_matrix',[],1);    
    for i_eps = 1 : n_eps
        x = sol_dro_exantereg{i_simu,i_eps}; 
        Was_dist = eps_set(i_eps);
        compute_maxreg_lb_CCG
        maxreg_lb_dro_exantereg_CCG(i_simu,i_eps) = lb_maxreg;        
    end
end
maxreg_lb_dro_exantereg_CCG = mean(maxreg_lb_dro_exantereg_CCG);
...
% dro_expostreg_CCG 
maxreg_lb_dro_expostreg_CCG = zeros(n_simu,n_eps);
for i_simu = 1 : n_simu    
    xi_hat_matrix = xi_matrix_true(sample_index{i_simu},:); 
    xi_hat_vec = reshape(xi_hat_matrix',[],1);      
    for i_eps = 1 : n_eps
        ...
        Was_dist = eps_set(i_eps);
        compute_maxreg_lb_CCG
        maxreg_lb_dro_expostreg_CCG(i_simu,i_eps) = lb_maxreg;
    end
end
maxreg_lb_dro_expostreg_CCG = mean(maxreg_lb_dro_expostreg_CCG);
...
% dro_cost_CCG 
maxreg_lb_dro_cost_CCG = zeros(n_simu,n_eps);
for i_simu = 1 : n_simu    
    xi_hat_matrix = xi_matrix_true(sample_index{i_simu},:); 
    xi_hat_vec = reshape(xi_hat_matrix',[],1);
    for i_eps = 1 : n_eps
        ...
        Was_dist = eps_set(i_eps); 
        compute_maxreg_lb_CCG
        maxreg_lb_dro_cost_CCG(i_simu,i_eps) = lb_maxreg;
    end
end
maxreg_lb_dro_cost_CCG = mean(maxreg_lb_dro_cost_CCG);
